...
import { getPathway, getPathwayNames, getNextState, formatGuidance } from '../pathways.js';
import { loadFreeExploreLog } from '../storage.js';
import type { FreeExploreEntry } from '../types.js';
...
export function registerAlwaysOnTools(server: McpServer, state: StateManager): void {
  ...
  const taskCreateHandle = server.registerTool(
    'task_create',
    {
      ...
      inputSchema: {
        ...
        pathway: z.string().optional().describe(`Workflow pathway: ${pathwayNames.join(', ')}`),
        generate_pathway: z.boolean().optional().describe('Set true to use a dynamic LLM-generated pathway instead of a static one'),
      },
    },
    async (args) => {
      ...
      if (task.pathway) {
        ...
        lines.push(`**Initial state:** \`enter_state("${pw.initial_state}", task_id="${task.id}")\``);
        ...
        lines.push('');
        lines.push('Each step declares: `label`, `base_state`, `description`, `acceptance_criteria`, `tools` (optional), `constraints` (optional).');
        lines.push('');
        lines.push('**Workflow:**');
        ...
        lines.push('3. Work the step. Use `pathway_prove(criteria_indices, proof_type, description)` to satisfy criteria.');
        ...
        lines.push('');
        lines.push('Generated pathways enforce per-step tool scoping and require empirical proof before advancement.');
        lines.push('Use `cortex_discover` to see which tools belong to which state before composing steps.');
      }
  ...
  const taskUpdateHandle = server.registerTool(
    'task_update',
    {
      ...
      inputSchema: {
        ...
        status: z.enum(['active', 'paused', 'blocked', 'completed', 'abandoned']).optional()
          .describe('New status'),
    ...
    async (args) => {
      ...
      if (result.error) return error(result.error);
      const t = result.task!;
      return success(`Task ${t.id} updated. Status: ${t.status}. Findings: ${t.findings.length}.`);
    },
  );
  ...
  const taskGetHandle = server.registerTool(
    'task_get',
    {
      description: 'Get full detail on a persistent task including all findings and state history.',
      inputSchema: {
        task_id: z.string().describe('Task ID'),
      },
    },
  ...
  const taskContextHandle = server.registerTool(
    'task_context',
    {
      description: 'Reconstruct context for a task -- description, state history, paginated findings. Use this to pick up where a previous session left off. Findings are paginated (50 per page).',
      inputSchema: {
        task_id: z.string().describe('Task ID'),
        page: z.preprocess(
        ...
        order: z.enum(['newest', 'oldest']).optional()
          .describe('newest = most recent findings on page 1 (default). oldest = chronological from start.'),
      },
    },
    async (args) => {
  ...
  const currentStateHandle = server.registerTool(
    'current_state',
    {
      description: 'Show current state, available tools, constraints, active task, and pathway guidance.',
    },
    async () => {
  ...
  const enterStateHandle = server.registerTool(
    'enter_state',
    {
      description: 'Transition to a workflow state. Tools for that state become available; other state tools disappear.',
      inputSchema: {
        state: z.enum(['base', 'recon', 'plan', 'implement', 'debug', 'validate', 'review', 'browse', 'free'])
  ...
  const freeExploreHandle = server.registerTool(
    'free_explore',
    {
      description: 'Ad-hoc escape hatch from any state. Temporarily unlocks all tools. Call `exit_free` to return to your previous state.',
      inputSchema: {
  ...
  const exitFreeHandle = server.registerTool(
    ...
    async () => {
      ...
      return success(`Returned to **${returnState}** state.`);
    },
  );
  ...
  const discoverHandle = server.registerTool(
    'cortex_discover',
    {
      description: 'Search all cortex tools across all states. In free explore, use this to find and enable specific tools.',
      inputSchema: {
        query: z.string().optional().describe('Search query -- matches tool names and state assignments'),
        enable: z.preprocess(
          (val) => typeof val === 'string' ? JSON.parse(val) : val,
          z.array(z.string()).optional(),
        ).describe('Tool names to enable (free explore only)'),
        disable: z.preprocess(
          (val) => typeof val === 'string' ? JSON.parse(val) : val,
          z.array(z.string()).optional(),
    ...
    async (args) => {
      ...
      const filtered = query
        ? allTools.filter(t => t.name.toLowerCase().includes(query) || t.state.toLowerCase().includes(query))
        : allTools;
      ...
      if (state.getCurrentState() === 'free') {
        lines.push('*Use `enable: ["tool_name"]` to activate tools, `disable: ["tool_name"]` to release them.*');
      }
  ...
  const exitStateHandle = server.registerTool(
    ...
    async () => {
      state.exitState();
      return success('Returned to **base** state.');
    },
  );
  ...
  const suggestStateHandle = server.registerTool(
    ...
    async (args) => {
      ...
      const keywords: Record<string, string[]> = {
        recon: ['understand', 'read', 'explore', 'investigate', 'learn', 'ticket', 'context', 'what is', 'how does'],
        plan: ['plan', 'design', 'architect', 'approach', 'strategy', 'rfc', 'proposal'],
        implement: ['code', 'build', 'implement', 'write', 'create', 'add', 'fix', 'change', 'modify', 'refactor'],
        debug: ['debug', 'error', 'bug', 'broken', 'failing', 'trace', 'log', 'issue', 'not working', '500', '404'],
        validate: ['test', 'verify', 'validate', 'e2e', 'prerequisite', 'local ci', 'flow check', 'sanity'],
      ...
      const hasMatch = top[0]?.score > 0;

      const lines = ['## State Suggestions', ''];
      for (const s of top) {
        const marker = s.score > 0 ? `(${s.score} matches)` : '(no keyword match)';
        lines.push(`- **${s.state}** ${marker}: ${s.description}`);
      }

      if (!hasMatch) {
        lines.push('');
        lines.push('No strong keyword match. Consider the **free_roam** pathway for open-ended work:');
        lines.push('`task_create(title, pathway="free_roam")` -- all tools, full state tracking.');
      }

      lines.push('', 'Use `enter_state` to transition.');
      return success(lines.join('\n'));
    },
  );
  ...
  const doctorHandle = server.registerTool(
    'cortex_doctor',
    {
      description: 'Health check. Tests proxied MCP server connections, reports tool counts, and flags tool names referenced in states.yaml that are not registered in any server.',
    },
    async () => {
  ...
  const freeExploreAnalysisHandle = server.registerTool(
    ...
    {
      description: 'Analyze the free explore audit log. Identifies recurring state escape patterns and suggests pathway additions.',
    },
    async () => {
      const entries = loadFreeExploreLog();
      if (entries.length === 0) {
        return success('No free explore entries recorded yet. Use `free_explore` during work to generate data.');
      }

      const byFromState = new Map<string, { count: number; reasons: string[]; totalDuration: number; entries: number }>();

      for (const entry of entries) {
        if (entry.duration_seconds !== null && entry.duration_seconds < 0) continue;

        const existing = byFromState.get(entry.from_state) || { count: 0, reasons: [], totalDuration: 0, entries: 0 };
        existing.count++;
      ...
      if (recurring.length > 0) {
        ...
        lines.push('These states are escaped frequently -- consider adding tools or pathway steps.');
      }
